...
class DatasetCollector:
    ...
    def scan(self):
        ...
        while retries < max_retries:
            # 1. Pede para a placa escanear fisicamente
            self.iface.scan()
            ...
            # 3. Cria um conjunto (set) com os pares (BSSID, Sinal) 
            # O set é perfeito pois ignora a ordem em que a placa retornou os dados
            current_scan_data = {(i.bssid, i.signal) for i in results if self.get_all_wifis or i.bssid in self.good_wifis}
            
            # Verificação A: Está vazio?
            if not current_scan_data:
                if self.verbose: print("⚠️ Retorno vazio. Tentando de novo...")
                retries += 1
                time.sleep(1) # Pequena pausa antes de forçar a placa novamente
                continue
                
            # Verificação B: É idêntico ao último scan? (Preso no cache)
            if hasattr(self, 'last_scan_data') and current_scan_data == self.last_scan_data:
                if self.verbose: print("♻️ Dados repetidos (cache detectado). Forçando novo scan...")
                retries += 1
                time.sleep(1)
                continue
                
            # Se passou pelas verificações, os dados são frescos! 
            # Salvamos esse scan como o "último" para comparar na próxima iteração
            self.last_scan_data = current_scan_data
            break # Sai do loop while
            
        # O processamento continua normalmente abaixo
        scaned = []
        t = time.time() - self.t0
        for i in results:
            if self.get_all_wifis or (i.bssid in self.good_wifis):
                bssid = i.bssid
                ssid = i.ssid
                signal = i.signal
