...
class Model:
    ...
    def __init__(self,
        ...
        self.x_building = None
        self.y_building = None
        self.power_prior_probability_distribution = {}

        self.trained = False

        self.building_polygon = np.load(self.path_to_the_polygon_files_of_each_building).T
        try:
            self.building_polygon_holes = np.load(self.path_to_the_polygon_files_of_each_building.replace('.npy', '_hole.npy')).T
        except FileNotFoundError:
            self.building_polygon_holes = None
        ...
        self.building_map_margin = 0.1  # meters
        self.max_power = 105
    ...
    def construct_building_map(self, X_train):
        ...
        building_polygon = self.building_polygons
        building_polygon_hole = self.building_polygons_holes
    ...
    def train(self, dataset):
            ...
            x_building, y_building = self.construct_building_map(dataset)
            print(x_building)

            self.x_building = x_building
            self.y_building = y_building
            ...
            for router in routers:
                ...
                if self.checking_non_null_minimum_percentage_of_samples(X_train, router):
                    ...
                    rbf_phi = Rbf(x_train, y_train, phi, function=self.radial_log_basis_function)
                    rbf_mu = Rbf(x_train, y_train, mu, function=self.radial_log_basis_function)
    ...
    def pred(self, X_test, selected_building=0, selected_floor=0):
        ...
        for index in X_test.index:
            ...
            for building in range(self.num_buildings):
                ...
                min_prob = self.eps * np.ones(len(self.x_building[building]))
                    ...
                    for router in self.get_valid_routers_building_and_floor(building, floor):
                        ...
                        if router in test_sample.columns:
                            ...
                            prob_xy_given_pbfr = prob_p_given_xybfr / (
                                    self.eps + self.power_prior_probability_distribution[building][floor][router][
                                power])
