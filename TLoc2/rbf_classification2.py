...
class TLoc:         
    ...
    def get_parameters_estimation(self, data, router):
        ...
        for class_ in self.classes:
            ...
            if len(data_of_router_in_class_without_zero_values) == 0:
                ...
                gmm.fit(data_of_router_in_class_without_zero_values[router].reshape(-1,1))
    ...
    def train(self): #### preciso arrumar ainda essa função
        for router in self.aps:
            ...
            for power in range(0, self.max_power):
                self.power_probability_masks[router][power] = self.approximate_position_density_function_given_router(power, np.array(mu), np.array(phi))
    ...
    def cumulative_distribution_function_of_gaussian_mixture(self, x, means, sigma, weights):
        return weights[0]*scipy.stat.norm.cdf(x, means[0], sigma) + weights[1]*scipy.stat.norm.cdf(x, means[1], sigma)
